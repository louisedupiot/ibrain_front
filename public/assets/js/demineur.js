...
var COULEUR_DRAPEAU = "#e3b947";
var COULEUR_CHIFFRE = "LightPink";
...
var changerTerrain = function () {
  ...
  if (
    ...
    y <= 0 ||
    y > 11 ||
    parseInt(nb_mine) == NaN ||
    nb_mine <= 0
  ) {
    ("Entrez des infos correctes");
    dessin = false;
  }
...
var clicBoutonAux = function (cx, cy) {
  ...
  if (cx >= 0 && cy >= 0 && cx < x && cy < y && affTerrain[cx][cy] == DEFAUT) {
    ...
    document.getElementById(cx + "" + cy).style.backgroundColor =
      COULEUR_CHIFFRE;
    ...
    if (terrain[cx][cy] == 0) {
      ...
      document.getElementById(cx + "" + cy).innerHTML = "&nbsp;";
    }
        ...
        for (j = 0; j < terrain.length; j++)
          if (terrain[i][j] == MINE) {
            ...
            document.getElementById(i + "" + j).innerHTML =
              "<img src=../assets/images/bobomb.png  width='30px' height='30px'>";
          }
      }
...
genererAutresCases = function () {
  for (var i = 0; i < x; i++) {
    for (var j = 0; j < y; j++) {
      terrain[i][j] =
        terrain[i][j] == undefined ? minesAutour(i, j) : terrain[i][j];
...
minesAutour = function (cx, cy) {
  ...
  // On regarde en haut à droite
  res +=
    cx == x - 1 || cy == y - 1 ? 0 : terrain[cx + 1][cy + 1] == MINE ? 1 : 0;
...
var testGagne = function () {
  for (var i = 0; i < x; i++) {
    for (var j = 0; j < y; j++) {
      if (
        affTerrain[i][j] == DEFAUT ||
        (affTerrain[i][j] == CLICK_DROIT && terrain[i][j] != MINE) ||
        (affTerrain[i][j] == CLICK_GAUCHE && terrain[i][j] == MINE)
      ) {
        return false;
      }
    }
